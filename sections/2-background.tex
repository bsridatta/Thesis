...
The \ac{JSD} It is hard to stabilize the \ac{gan}'s minimax game \cite{martin2017principled}. It requires carefully tuned hyperparameters to maintain an equilibrium between the two players. Failing to find the proper balance betweem the networks leads to the problem of \textit{Non-Convergence} where the training oscillates and never converge. When the generator is not strong enough and learns to produce samples that fool the discriminator, it eventually would restrict itself to only learn to produce such samples. This problem is refered to as \textit{Mode Collapse} \cite{commonganprobs}.



\subsection{WGAN}
\acp{wgan} is a variant of \acp{gan} where the second network is not a discriminator that predicts binary labels of 1 and 0 for real or fake, but is a critic   that scores the samples on how real they look.
