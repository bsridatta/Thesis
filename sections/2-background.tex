...
The first term is the above equation makes sure the reconstruction of the data sample $x$, while the second term tries to keep the learnt distribution $q(z|x)$ close to the true prior $p(z)$. Hence the loss term to \textit{minimize} while training the vae is $\mathcal{L}_{\mathrm{VAE}}$ \ref{eqn:vae_loss}.
