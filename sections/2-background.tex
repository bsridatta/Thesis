...
Autoencoder is a variant of \acp{ann} that is designed to learn an identity function that generates the input back. The network has a bottleneck($z$), dividing the network into two parts, an encoder and a decoder as illustrated in fig\ref{fig:ar_arch}. The former learns to compress the high demensional input to a low dimensional intermediate representation at the bottleneck, while the latter learns to reconstruct the input using the low dimensional input alone. The network thus efficiently learns to compressing data which is used for dimensionality reduction and feature learning. In recent years, autoencoders are popularly used as generative networks leveraging their ability to learn feature representations in an unsupervised way. 

The network is trained to minimize the similarity between the reconstruction and the original input. This similarity can be determined by metrics such as \ac{MSE}, \ac{L1} or cross-entropy loss. In the process of learning to reconstruct the original data, the encoder learns to filter the most important features of the given data distribution, so as it preserve the complete properties within the limit of the bottleneck. While the decoder learns the general properties of the distribution which with the 
